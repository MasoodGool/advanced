...
import { users, listings, escrows, stats, createEscrow, resetStore } from './store';
import { ship, confirm, dispute, resolve, cancel } from './escrowMachine';

const router = Router();

router.get('/state', (req, res) => {
  res.json({ users, listings, escrows, stats });
});
...
router.post('/escrow', (req, res) => {
  ...
  if (buyer.balance < listing.price)  return res.status(400).json({ error: `Insufficient balance. Need R\u00A0${listing.price.toLocaleString()}, have R\u00A0${buyer.balance.toLocaleString()}.` });
...
router.post('/escrow/:id/ship', (req, res) => {
  const escrow = getEscrow(req, res); if (!escrow) return;
  try { ship(escrow, req.body.userId); res.json({ escrow }); }
  catch (e) { res.status(400).json({ error: e.message }); }
});

router.post('/escrow/:id/confirm', (req, res) => {
  const escrow = getEscrow(req, res); if (!escrow) return;
  try { confirm(escrow, req.body.userId); res.json({ escrow, users, stats }); }
  catch (e) { res.status(400).json({ error: e.message }); }
});

router.post('/escrow/:id/dispute', (req, res) => {
  const escrow = getEscrow(req, res); if (!escrow) return;
  try { dispute(escrow, req.body.userId, req.body.reason); res.json({ escrow }); }
  catch (e) { res.status(400).json({ error: e.message }); }
});

router.post('/escrow/:id/resolve', (req, res) => {
  const escrow = getEscrow(req, res); if (!escrow) return;
  try { resolve(escrow, req.body.decision); res.json({ escrow, users, stats }); }
  catch (e) { res.status(400).json({ error: e.message }); }
});

router.post('/escrow/:id/cancel', (req, res) => {
  const escrow = getEscrow(req, res); if (!escrow) return;
  try { cancel(escrow, req.body.userId); res.json({ escrow, users }); }
  catch (e) { res.status(400).json({ error: e.message }); }
});
